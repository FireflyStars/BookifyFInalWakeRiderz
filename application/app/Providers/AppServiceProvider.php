        ...
        if(DB::connection()->getDatabaseName())
        {
            if(Schema::hasTable('settings'))
            {
                ...
                if($settings)
                {
                    ...
                    elseif($settings->default_currency=="GBP")
                    {
                        Config::set('settings.currency_symbol', '&pound;');
                    }
                    elseif($settings->default_currency=="EUR")
                    {
                        Config::set('settings.currency_symbol', '&euro;');
                    }
                    elseif($settings->default_currency=="ARS" || $settings->default_currency=="MXN")
                    {
                        Config::set('settings.currency_symbol', '&#36;');
                    }
                    elseif($settings->default_currency=="BRL")
                    {
                        Config::set('settings.currency_symbol', '&#82;&#36;');
                    }
                    elseif($settings->default_currency=="DKK"  || $settings->default_currency=="SEK"  || $settings->default_currency=="NOK")
                    {
                        Config::set('settings.currency_symbol', '&#107;&#114;');
                    }
                    elseif($settings->default_currency=="INR")
                    {
                        Config::set('settings.currency_symbol', '&#8377;');
                    }
                    elseif($settings->default_currency=="CHF")
                    {
                        Config::set('settings.currency_symbol', '&#67;&#72;&#70;');
                    }
                    elseif($settings->default_currency=="JPY")
                    {
                        Config::set('settings.currency_symbol', '&#165;');
                    }
                    elseif($settings->default_currency=="RUB")
                    {
                        Config::set('settings.currency_symbol', '&#1088;&#1091;&#1073;');
                    }
                    elseif($settings->default_currency=="TRY")
                    {
                        Config::set('settings.currency_symbol', '&#8356;');
                    }
