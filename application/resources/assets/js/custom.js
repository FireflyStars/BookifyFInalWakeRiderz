...
$(document).ready(function(){
    ...
    $('input[id="event_date"]').change(function () {
        ...
        var staff_member_id;
        staff_member_id = $('select[id="staff_member_id"]').val();
        ...
        $.ajax({
            ...
            data: {event_date:selected_date,staff_member_id:staff_member_id},
            beforeSend: function() {
    ...
    $('select[id="staff_member_id"]').change(function () {
        //populate timing slots
        var staff_member_id;
        staff_member_id = $(this).val();

        var selected_date;
        selected_date = $('#event_date').val();
    ...
    $('#booking_step_2').submit(function(e){
        ...
        if(staff_member_id === "" || staff_member_id === null)
        {
            $('#staff_member_id').addClass('is-invalid');
            $('#staff_error_holder').removeClass('d-none');
            $("html, body").animate({ scrollTop: 2000 }, "slow");
            check = false;
        }
    ...
    $('#promo_code_form').submit(function(e){
        $form = $(this);
        $('.promo_notification').addClass('d-none');
        ...
        {
            //show error
            $('.promo_notification').html('Please provide a code.').removeClass('d-none');
            $form.find('button').prop('disabled',false);
            return false;
        }
        else
        {
            //send ajax request to verify code and give discount
            $.ajax({
                type: 'POST',
                url: URL_CONCAT + '/verify-promo-code',
                data: {code:code},
                beforeSend: function () {
                    //show button loader and disable button
                    $('#promo_loader').removeClass('d-none');
                    $form.find('button').prop('disabled',true);
                },
                success: function(response) {
                    if(response === "404")
                    {
                        //show error
                        $('.promo_notification').html('Not a valid promo code.').removeClass('d-none');
                        $('#promo_loader').addClass('d-none');
                        $form.find('button').prop('disabled',false);
                    }
                    else if(response === "max_used")
                    {
                        //show error
                        $('.promo_notification').html('This promo code is fully used.').removeClass('d-none');
                        $('#promo_loader').addClass('d-none');
                        $form.find('button').prop('disabled',false);
                    }
                    ...
                    {
                        //show error
                        $('.promo_notification').html('This promo code is expired.').removeClass('d-none');
                        $('#promo_loader').addClass('d-none');
                        $form.find('button').prop('disabled',false);
                    }
                    else if(response === "wrong_category")
                    {
                        //show error
                        $('.promo_notification').html('Your booking type is not valid for this promo code.').removeClass('d-none');
                        $('#promo_loader').addClass('d-none');
                        $form.find('button').prop('disabled',false);
                    }
                    else
                    {
                        $('#pricing_holder').html(response);
                        location.reload();
                    }
                }
            });
